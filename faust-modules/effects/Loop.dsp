...
B = hslider("Start/Stop Recording (Max 20s)[acc:1 0 -10 12 0 1]",0,0,1,1);	// Capture sound while pressed
...
capture = *(B) : (+ : delay(1048576, D-1)) ~ *(1.0-B) ;


smooth(c)	= *(1-c) : +~*(c);
level		= hslider("Volume [unit:dB]", 0, -96, 4, 0.1) : db2linear : smooth(0.999);
