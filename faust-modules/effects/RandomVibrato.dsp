...
vibratoGain = hslider("[2]Vibrato Intensity[style:knob][acc:1 0 -10 10 0 0.1]", 0.4, 0.05, 0.4, 0.01) : smooth(0.999);
vibratoFreq = vfreq; 
vibratoSpeed = hslider("[1]Vibrato Speed Range[scale:log][acc:2 0 -10 10 0 10]", 10, 5, 40, 1) : smooth(0.99) : min(40) : max(1);
...
speed = hslider ("[3]Occurrence Speed (Granulator)[unit:Hz][style:knob][acc:0 1 -10 10 0 4]",1,0.001,10,0.0001):lowpass(1,1);
proba = hslider ("[4]Probability (Granulator)[unit:%][style:knob][acc:0 1 -10 10 0 88]",88,75,100,1) *(0.01):lowpass(1,1);
