...
ringmod = _<:_,*(oscs(freq)):drywet
		with {
				freq = hslider ( "[1]Modulation Frequency[acc:1 0 -10 0 10][scale:log]", 2,0.001,100,0.001):smooth(0.999);	
				drywet(x,y) = (1-c)*x + c*y;
				c = hslider("[2]Modulation intensity[style:knob][unit:%][acc:1 0 -10 0 10]", 60,0,100,0.01)*(0.01):smooth(0.999);
			};
