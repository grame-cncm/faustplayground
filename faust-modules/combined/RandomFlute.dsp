...
pressure = 1;
breathAmp = hslider("h:[3]Parameters/Breath Noise[style:knob][acc:0 1 -10 10 0 0.02]", 0.02, 0.01, 0.05, 0.0001):smooth(0.999):min(0.05):max(0.01);
...
echo = +~ @(22050) *(feedback);
feedback = hslider("h:Parameters/Echo Intensity[style:knob][acc:2 0 -10 10 0 0.001] ", 0.001, 0.001, 0.65, 0.001):smooth(0.999);
...
speed = hslider ("h:[1]Pulse/[1]Speed (Granulator)[unit:Hz][style:knob][acc:0 1 -10 10 0 3]",1,1,6,0.0001):lowpass(1,1);
proba = hslider ("h:[1]Pulse/[2]Probability (Granulator)[unit:%][style:knob][acc:0 1 -10 10 0 88]",88,60,100,1) *(0.01):lowpass(1,1);
...
instrReverbFlute = zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       roomSize = hslider("h:Reverb/Reverberation Room Size [style:knob][acc:1 1 -10 10 0 0.2]",0.2,0.01,1.7,0.01);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
