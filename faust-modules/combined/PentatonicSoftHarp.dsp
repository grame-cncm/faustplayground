...
beta = 0.5;
t60 = hslider("h:[3]Reverb/[1]Resonance[unit:s][acc:2 0 -10 0 10]", 5, 0.5, 10, 0.01);  // -60db decay time (sec)
...
zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       reverbGain = hslider("h:[3]Reverb/[1]Reverberation Volume(InstrReverb)[style:knob][acc:1 0 -10 0 10]", 0.2,0.05,1,0.01):smooth(0.999):min(1):max(0.05);
       roomSize = hslider("h:[3]Reverb/[2]Reverberation Room Size (InstrReverb)[style:knob][acc:1 0 -10 0 10]", 0.2,0.05,1.7,0.01):min(1.3):max(0.05);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
