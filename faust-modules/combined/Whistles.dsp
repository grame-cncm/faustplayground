...
freq(0) = hslider("[1]Frequency 0[unit:Hz][acc:2 1 -10 10 0 110]", 110, 50, 220, 0.01):smooth(0.999);
freq(1) = hslider("[2]Frequency 1[unit:Hz][acc:2 1 -10 10 0 400]", 400, 220, 660, 0.01):smooth(0.999);
freq(2) = hslider("[3]Frequency 2[unit:Hz][acc:2 1 -10 10 0 820]", 820, 660, 1100, 0.01):smooth(0.999);
...
vibratoGain = 0.17;//hslider("Vibrato Volume[style:knob][acc:1 0 -10 10 0 0.1]", 0.1, 0.05, 0.5, 0.01) : smooth(0.999);
vibratoFreq = vfreq; //hslider("Vibrato Frequency[unit:Hz][acc:0 0 -10 12 0 5]", 3, 0, 10, 0.001) : smooth(0.999);
...
speed = 0.5;
proba = 0.9; //hslider ("h:Pulse/Probability[unit:%][style:knob][acc:1 1 -10 10 0 88]",88,75,100,1) *(0.01):lowpass(1,1);
...
roomsizeSlider  = hslider("[7]Reverberation Room Size (Freeverb)[style:knob][acc:1 1 -10 13 0 0.5]", 0.5, 0.1, 0.9, 0.025) : smooth(0.999) : min(0.9) :max(0.1) *scaleroom + offsetroom;
wetSlider       = hslider("[6]Reverberation Intensity (Freeverb)[style:knob][acc:1 1 -10 15 0 0.3333]", 0.3333, 0.1, 0.9, 0.025) : smooth(0.999) : min(0.9) :max(0.1);
