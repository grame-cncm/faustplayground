...
process = vgroup("Harp", h : harpe(C,N,K) : instrReverbHarp : *(l),*(l))
	with {
		...
		h = hslider("[1]Instrument Hand[1] [acc:0 1 -10 10 0 24]", 1, 0, N, 1) : int: automat(bps, 15, 0.0)
			with{
...
zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       reverbGain = hslider("h:[3]Reverb/ Reverberation Volume (InstrReverb)[style:knob][acc:1 1 -10 10 0 0.2]",0.2,0.05,1,0.01):smooth(0.999):min(1):max(0.05);
       roomSize = hslider("h:[3]Reverb/Reverberation Room Size  (InstrReverb)[style:knob][acc:1 1 -10 10 0 0.2]",0.2,0.05,1.3,0.01):min(1.3):max(0.05);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
