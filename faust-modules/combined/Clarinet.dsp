...
freq = hslider("h:[2]Instrument/Frequency[unit:Hz][tooltip:Tone frequency][acc:1 0 -14 0 12]", 440,110,1300,0.01):smooth(0.999);
gain = 1;
...
noiseGain = hslider("h:[3]Parameters/Breath Noise[style:knob][acc:0 0 -12 0 12]", 0.02,0,0.12,0.01);
pressure = hslider("h:[3]Parameters/ Pressure[style:knob][acc:1 1 -5 0 10]", 0.8,0.65,1,0.01);
...
zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       reverbGain = hslider("h:[4]Reverb/ Reverberation Volume (InstrReverb)[style:knob][acc:1 0 -15 0 15]", 0.137,0.05,1,0.01) :smooth(0.999):min(1):max(0.05);
       roomSize = hslider("h:[4]Reverb/Reverberation Room Size (InstrReverb)[style:knob][acc:1 0 -15 0 15]", 0.45,0.05,2,0.01):min(2):max(0.05);   
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
