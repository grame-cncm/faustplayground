...
process = vgroup("Harp", h : harpe(C,N,K) :>  instrReverbHarp : *(l),*(l))
	with {
		...
		h = hslider("[1]Instrument Hand[acc:0 1 -10 10 0 11]", 1, 0, N, 1) : int: automat(bps, 15, 0.0)
			with{
			bps = hslider("h:[2]Parameters/[1]Speed[style:knob][acc:0 1 -12 10 0 480]", 480, 180, 720, 1):smooth(0.999) : min(720) : max(180) : int;
			};
		l = 0.9;
		C = 0.5;	
		};
...
harpe(C,N,b) = 	_ <: par(i, N, position(i+1)
				 	...
				 	:> _,_
	with {
		att  = hslider("h:[2]Parameters/[2]Resonance[style:knob][acc:2 1 -12 10 0 5]", 5, 0.1, 10, 0.01):min(10):max(0.1); 
		lvl  = 1;
...
zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       reverbGain = hslider("h:[3]Reverb/[1]Reverberation Volume(InstrReverb)[style:knob][acc:1 1 -10 10 0 0.2]",0.2,0.05,1,0.01):smooth(0.999):min(1):max(0.05);
       roomSize = hslider("h:[3]Reverb/[2]Reverberation Room Size (InstrReverb)[style:knob][acc:1 1 -10 10 0 0.2]",0.2,0.05,1.3,0.01):min(1.3):max(0.05);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
