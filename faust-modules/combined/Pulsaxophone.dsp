...
freq = hslider("[1]Frequency[unit:Hz][acc:1 1 -12 10 0 110]",440,80,880,1):smooth(0.9999):min(880):max(80);
gain = 0.8;
gate = pulsaxo.gate;
...
vibratoFreq = hslider("[3]Vibrato Frequency[style:knob][unit:Hz][acc:0 1 -10 10 0 6]",6,1,15,0.1):smooth(0.999);
vibratoGain = 0.2;
vibratoBegin = 0.05;
vibratoAttack = 0.03;
vibratoRelease = 0.1;
...
speed = hslider ("h:[2]Pulse/[1]Speed (Granulator)[unit:Hz][style:knob][acc:0 1 -10 10 0 4]",1,0.001,7,0.0001):lowpass(1,1);
proba = hslider ("h:[2]Pulse/[2]Probability (Granulator)[unit:%][style:knob][acc:1 0 -10 10 0 88]",88,75,100,1)*(0.01):lowpass(1,1);
...
instrReverbAccel = zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       roomSize = hslider("h:[4]Reverb/Reverberation Room Size (InstrReverb)[style:knob][acc:1 1 -15 12 0 0.72]",0.72,0.1,1.7,0.01):min(1.7):max(0.1);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
