...
autoTrig = beat(t) * (abs(noise) <= p) : trigger(48) //tempo(2.5*t))
	with {
		t = hslider("[1]Speed (Granulator)[style:knob][acc:0 1 -10 10 0 240]", 240, 120, 480, 0.1) : smooth(0.999);
		p = hslider("[2]Probability (Granulator)[unit:%][style:knob][acc:1 0 -10 10 0 50]", 50, 25, 100, 1)*(0.01) : smooth(0.999);
...
with {
  ...
  presence = hslider("[3]Proximity (InstrReverb)[style:knob][acc:1 0 -15 10 0 0.5]", 0.5, 0, 1, 0.01) : smooth(0.999);
