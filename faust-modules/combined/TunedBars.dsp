...
B = hslider("[3]Speed[style:knob][acc:0 0 -10 0 10]", 360, 120, 720, 60): smooth(0.99) : min(720) : max(120) : int;
...
echo = +~(@(22050)*(feedback));
//feedback = hslider("Echo Intensity (Feedback)[acc:1 0 -5 0 12]", 0.1, 0.05, 0.65, 0.01):smooth(0.999):min(0.05):max(0.65);
feedback = 0.8;
drywet(x,y) 	= (1-c)*x + c*y
				with {
					c = hslider("[4]Echo Intensity[style:knob][unit:%][acc:1 0 -8 0 10]", 20,0,99,0.01)*(0.01):smooth(0.999):min(0.99):max(0.001);
					};
					
//instrReverb from instrument.lib
instrReverbChime = zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+       
	with{
       roomSize = hslider("h:[5]Reverb/Reverberation Room Size (InstrReverb)[style:knob][acc:1 0 -10 0 12]", 0.2,0.1,1.7,0.01):min(1.7):max(0.1);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
