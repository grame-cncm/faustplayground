...
trigger(n) = position(n): trig
	...
	decay(n,x)	= x - (x>0.0)/n;
	release(n)	= + ~ decay(n);
	noteDuration = hslider("h:[1]/[3]Note Duration[unit:s][style:knob][acc:2 1 -10 10 0 0.166]", 0.166, 0.1, 0.25, 0.01)*44100 : min(11025) : max(4410):int;
	trig = upfront : release(noteDuration) : >(0.0);
	};

position(n) = abs(hand - n) < 0.5;
hand = hslider("h:[1]/[1]Instrument Hand[acc:0 1 -12 10 0 9]", 8, 0, N, 1): automat(bps, 15, 0.0)// => gate
		with{
		bps = hslider("h:[1]/[2]Speed[style:knob][acc:0 1 -10 10 0 480]", 480, 180, 720, 1):smooth(0.999) : min(720) : max(180) : int;
		};
//----------------------- INSTRREVERB ----------------------------
...
zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       reverbGain = hslider("h:[2]Reverb/[1]Reverberation Volume (InstrReverb)[style:knob][acc:1 1 -10 10 0 0.2]",0.2,0.05,1,0.01):smooth(0.999):min(1):max(0.05);
       roomSize = hslider("h:[2]Reverb/[2]Reverberation Room Size (InstrReverb)[style:knob][acc:1 1 -10 10 0 0.5]",0.5,0.05,2,0.01):min(2):max(0.05);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
