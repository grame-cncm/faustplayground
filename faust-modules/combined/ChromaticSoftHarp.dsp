...
N = 24;
hand = hslider("h:[1]/Instrument Hand[acc:0 1 -10 10 0 12]", 12, 0, N, 1) : automat(bps, 15, 0.0)// => gate
with{
...
t60 = hslider("h:[2]Reverb/[1]Resonance (InstrReverb)[unit:s][acc:2 1 -10 10 0 5]", 5, 0.5, 10, 0.01):min(10):max(0.5);  // -60db decay time (sec)
B = 0;
L = -10 : db2linear;
...
zita_rev1_stereo(rdel,f1,f2,t60dc,t60m,fsmax),_,_ <: _,!,_,!,!,_,!,_ : +,+
       with{
       reverbGain = hslider("h:[2]Reverb/[2]Reverberation Volume (InstrReverb)[style:knob][acc:1 1 -30 17 0 0.2]",0.2,0.05,1,0.01):smooth(0.999):min(1):max(0.05);
       roomSize = hslider("h:[2]Reverb/[3]Reverberation Room Size (InstrReverb)[style:knob][acc:1 1 -30 16 0 0.72]",0.72,0.05,2,0.01):min(2):max(0.05);
       rdel = 20;
       f1 = 200;
       f2 = 6000;
       t60dc = roomSize*3;
