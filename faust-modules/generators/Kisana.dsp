...
kisana = vgroup("Kisana", harpe(C,11,48), harpe(C,11,60), (harpe(C,11,72) : *(1.5), *(1.5)) 
	...
	with {
		...
		C = hslider("[2]Brightness[acc:0 1 -10 10 0 0.2]",0, 0, 1, 0.01) : automat(BPS, CCY, 0.0);
	};
...
harpe(C,N,b) = 	hand(b) <: par(i, N, position(i+1)
				 	...
				 	:> _,_
	with {
		att  = hslider("[3]Resonance[acc:2 1 -10 12 0 4]", 4, 0.1, 10, 0.01); 
		hand(48) = vslider("h:[1]Instrument Hands/1 (Note %b)[unit:pk][acc:1 0 -10 14 0 0]", 0, 0, N, 1) : int : automat(120, CCY, 0.0);
		hand(60) = vslider("h:[1]Instrument Hands/2 (Note %b)[unit:pk][acc:1 0 -10 14 0 2]", 0, 0, N, 1) : int : automat(240, CCY, 0.0);
		hand(72) = vslider("h:[1]Instrument Hands/3 (Note %b)[unit:pk][acc:1 0 -10 10 0 4]", 0, 0, N, 1) : int : automat(480, CCY, 0.0);
