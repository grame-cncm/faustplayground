declare name "Chromatic Tuned Bars";
...
process = par(i, N, tunedBar(i)):> lowpass(1,5000);
...
hand = hslider("[1]Instrument Hand[acc:0 1 -10 10 0 12]", 12, 0, N, 1):smooth(0.999):min(N):max(0):int:automat(B, 15, 0.0);
B = hslider("[2]Speed[style:knob][acc:0 1 -10 10 0 480]", 480, 180, 720, 60): smooth(0.99) : min(720) : max(180) : int;
