...
hold = hslider("[2]Hold[acc:1 1 -8 12 0 0]", 0, 0, 1, 1);
gate = hand : upfront : counter <=(0)
with{
hand = hslider("[1]Instrument hand[acc:1 0 -10 10 0 5]", 5, 0, 10, 1):automat(bps, 15, 0.0);
bps = hslider("[3]Speed[style:knob][acc:0 1 -10 10 0 480]", 480, 120, 720, 1) : smooth(0.999) : min(720) : max(120) : int;
upfront(x) = abs(x-x') > 0;
counter(g) = (+(1):*(1-g))~_;
};