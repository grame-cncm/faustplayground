...
/// <reference path="../Utilitary.ts"/>
class ModuleView {
    constructor(ID, x, y, name, htmlParent) {
    ...
    constructExtras(ID, name, fModuleContainer) {
        if (name == "input") {
...
"use strict";
class Module {
    constructor(id, x, y, name, htmlElementModuleContainer, removeModuleCallBack, compileFaust, audioContext) {
    ...
    drawInterface(id, x, y, name, container) {
    ...
    setFaustInterfaceControles() {
        ...
        var moduleFaustInterface = new FaustInterfaceControler((faustInterface) => { this.interfaceSliderCallback(faustInterface); }, (adress, value) => { this.moduleFaust.fDSP.setValue(adress, value); });
        ...
        ********************************************************************/
        ...
        this.setDownloadOptions = (serverUrl, shaKey, plateforme, architecture, appType) => {
            if (shaKey.indexOf("ERROR") == -1) {
                ...
                linkDownload.textContent = _("Download");
...
class Broadcast {
    constructor(app) {
        ...
        this.players.setSendFunc((msg) => this.send(msg));
        var wsurl = ((location.protocol === 'http:') ? 'ws://' : 'wss://') +
    ...
    send(msg) {
        switch (this.ws.readyState) {
            case WebSocket.CONNECTING:
                ...
                this.ws.addEventListener('open', () => this.send(msg));
                break;
            case WebSocket.OPEN:
                ...
                break;
            default:
                console.error('Unable to send message with a websocket at this status:', this.ws.readyState, msg);
        }
    }
    ...
    onRequestOffer(msg) {
        this.players.me.createOfferFor(this.players.getPlayer(msg.from), this.players.getOutputStream(), this.app.getRTCConfiguration());
    }
}
class WSMessage {
...
class Scene {
    ...
    /*********************** MUTE/UNMUTE SCENE ***************************/
    muteScene() {
        if (this.app.audioContext.suspend) {
    ...
    delayedUnmuteScene() {
        if (this.app.audioContext.resume) {
    ...
    integrateInput() {
        ...
        this.fAudioInput = new Module(Utilitary.idX++, positionInput.x, positionInput.y, "input", this.sceneView.inputOutputModuleContainer, (module) => { this.removeModule(module); }, this.compileFaust, this.app.audioContext);
        this.fAudioInput.patchID = "input";
        this.compileFaust({ name: "input",
            sourceCode: "process=_,_;",
            x: positionInput.x,
            y: positionInput.y,
            callback: (factory) => { this.integrateAudioInput(factory); }
        });
    }
    integrateOutput() {
        var positionOutput = this.positionOutputModule();
        this.fAudioOutput = new Module(Utilitary.idX++, positionOutput.x, positionOutput.y, "output", this.sceneView.inputOutputModuleContainer, (module) => { this.removeModule(module); }, this.compileFaust, this.app.audioContext);
...
********************************************************************/
    activateAudioInput() {
        ...
        if (navigator.getUserMedia) {
            navigator.getUserMedia({ audio: true }, (mediaStream) => { this.connectInputStream(mediaStream); }, (e) => {
    ...
    connectInputStream(stream) {
        ...
        var src = this.app.audioContext.createMediaStreamSource(stream);
    ...
    activateAudioOutput(sceneOutput) {
        ...
        var out = this.app.audioContext.destination;
        var ctor = new Connector();
        ctor.connectOutput(sceneOutput, out);
    }
    activateBroadcast(sceneOutput) {
        var ctor = new Connector();
        var stream = ctor.getBroadcastStream(sceneOutput, this.app.audioContext);
        if (stream) {
    ...
    createModule(factory) {
        try {
            ...
            var module = new Module(Utilitary.idX++, this.tempModuleX, this.tempModuleY, this.tempModuleName, document.getElementById("modules"), (module) => { this.removeModule(module); }, this.compileFaust, this.app.audioContext);
...
class LibraryView {
    initLibraryView() {
        ...
        instrumentLibraryTitle.appendChild(document.createTextNode(_("Instruments")));
        ...
        effetLibraryTitle.appendChild(document.createTextNode(_("Effects")));
        ...
        exempleLibraryTitle.appendChild(document.createTextNode(_("Examples")));
...
class AccelerometerEditView {
    ...
    initAccelerometerEdit() {
        ...
        label1.id = "curve1";
        label1.textContent = _("curve 1");
        var label2 = document.createElement("label");
        label2.className = "curve";
        label2.id = "curve2";
        label2.textContent = _("curve 2");
        var label3 = document.createElement("label");
        label3.className = "curve";
        label3.id = "curve3";
        label3.textContent = _("curve 3");
        var label4 = document.createElement("label");
        label4.className = "curve";
        label4.id = "curve4";
        label4.textContent = _("curve 4");
        ...
        checkOnOffLabel.textContent = _("enable/disable accelerometer");
...
class PlayerMenuItem {
    constructor(player, container) {
        ...
        d3.select(this.element)
            ...
            .on('dblclick', () => {
            document.dispatchEvent(new CustomEvent('dbltouchplayer', { 'detail': this }));
        });
    }
    refresh() {
        d3.select(this.element)
...
class Player {
    constructor(ident, send) {
        this.ident = ident;
        this.nickname = ident;
        this.send = send;
    }
    requestOfferFrom(player) {
    ...
    createOfferFor(player, stream, conf) {
        ...
        this.pc.createOffer({ mandatory: { OfferToReceiveAudio: true, OfferToReceiveVideo: false } }).then((desc) => {
            this.pc.setLocalDescription(desc).then(() => this.send(new WSMessage('Offer', undefined, player.ident, desc)));
        });
    }
    createAnswer(msg, dest_player, conf) {
        console.log('Je vais répondre à cette offre:', msg);
        this.pc = new RTCPeerConnection(conf);
        this.pc.onaddstream = (mse) => {
        ...
        this.pc.setRemoteDescription(msg.payload).then(() => {
            ...
            this.pc.createAnswer().then((desc) => {
                this.pc.setLocalDescription(desc).then(() => this.send(new WSMessage('Answer', undefined, msg.from, desc)));
            });
        }, () => console.error('setRemoteDescription a échoué'));
    }
    applyAnswer(msg) {
        this.pc.setRemoteDescription(msg.payload).then(() => console.log('Réponse reçue et prise en compte'), () => console.error("La réponse n'a pu être acceptée", msg));
    }
    addICECandidate(msg) {
        this.pc.addIceCandidate(new RTCIceCandidate(msg.payload)).then(() => console.info('ICE candidate correctement ajouté'), () => console.error("ICE candidate n'a pas pu être ajouté", msg.payload));
    }
    ...
    updateNickname(nickname) {
        ...
        if (this.module)
            this.module.moduleView.refresh();
    }
    notifyDisconnected() {
...
class Players {
    ...
    onPlayerDisconnected(msg) {
        var indent = msg.from;
        var player = this.index[indent];
        delete this.index[indent];
        player.notifyDisconnected();
    }
    getOrCreatePlayer(ident, no_ui = false) {
        if (this.index[ident]) {
            var player = this.index[ident];
            if (!player.getMenuItem())
                player.setMenuItem(new PlayerMenuItem(player, this.app.menu.menuView.playersContent));
            return player;
        }
        var player = new Player(ident, this.send);
    ...
    createMePlayer(msg) {
        ...
        this.getNickname().then((nickname) => {
            this.me = this.getOrCreatePlayer(msg.payload, true);
    ...
    getNickname() {
        ...
        return new Promise((resolve, reject) => {
            var form = content.append('form')
                ...
                evt.preventDefault();
                evt.stopPropagation();
                sessionStorage.setItem('nickname', ((evt.target).elements.namedItem('nickname')).value);
    ...
    updatePlayerNickname(msg) {
        var player = this.getOrCreatePlayer(msg.from);
        player.updateNickname(msg.payload);
    }
    onModulePlayerRemoved(player) {
...
class PlayerModule extends Module {
    constructor(id, x, y, name, container, removeModuleCallBack, compileFaust, audioContext, player) {
...
class PlayerModuleView extends ModuleView {
    constructor(ID, x, y, name, parent, player) {
        ...
        d3.select(this.fModuleContainer)
            .select('div.content')
            .text(this.player.nickname);
        this.closeButton = (d3.select(this.fModuleContainer)
            .append('div')
            .attr('class', 'close')
            .attr('draggable', false)
            .node());
    }
    constructExtras(ID, name, container) { }
    refresh(player) {
        if (player)
            this.player = player;
        d3.select(this.fModuleContainer)
...
(function (MenuChoices) {
    ...
    MenuChoices[MenuChoices["players"] = 8] = "players";
})(MenuChoices || (MenuChoices = {}));
class Menu {
    constructor(htmlContainer, app) {
        ...
        this.menuView.playersButton.addEventListener('click', () => this.menuHandler(this.newMenuChoices = MenuChoices.players));
        this.menuView.playersButton.querySelector('input[type=checkbox]').addEventListener('click', (e) => this.enableBroadcast(e));
        ...
        this.menuView.cleanButton.addEventListener("click", () => {
            new Confirm(_("Do you really want to empty the scene?"), (callback) => { this.cleanScene(callback); });
        });
        ...
        document.addEventListener("clouderror", (e) => { Menu.connectionProblem(e); });
    ...
    static updateSelectLocal(select) {
        Menu.clearSelect(select);
        Menu.fillSelectLocal(select);
    }
    //fill select box
    static fillSelectLocal(select) {
        if (typeof sessionStorage != 'undefined') {
            for (var i = 0; i < localStorage.length; i++) {
                var option = document.createElement("option");
    ...
    static connectionProblem(event) {
...
class MenuView {
    constructor(htmlContainer) {
        ...
        libraryButtonMenu.appendChild(document.createTextNode(_("Library")));
        ...
        exportButtonMenu.appendChild(document.createTextNode(_("Export")));
        ...
        helpButtonMenu.appendChild(document.createTextNode(_("Help")));
        ...
        editButtonMenu.appendChild(document.createTextNode(_("Edit")));
        ...
        loadButtonMenu.appendChild(document.createTextNode(_("Load")));
        ...
        saveButtonMenu.appendChild(document.createTextNode(_("Save")));
        ...
        this.HTMLButtonsMenu.push(libraryButtonMenu, this.playersButton, loadButtonMenu, saveButtonMenu, exportButtonMenu, helpButtonMenu);
        ...
        this.HTMLElementsMenu.push(libraryContent, this.playersContent, loadContent, saveContent, exportContent, helpContent);
...
class App {
    ...
    ********************************************************************/
    compileFaust(compileFaust) {
        ...
        var args = ["-I", /[^#]*/.exec(location.href) + "faustcode/"];
    ...
    createModule(factory) {
        ...
        var module = new Module(Utilitary.idX++, this.tempModuleX, this.tempModuleY, this.tempModuleName, document.getElementById("modules"), (module) => { this.scene.removeModule(module); }, this.compileFaust, this.audioContext);
        module.moduleFaust.setSource(this.tempModuleSourceCode);
        module.createDSP(factory);
        module.setFaustInterfaceControles();
        module.createFaustInterface();
    ...
    highlightPossibleTarget(module, module_type, over) {
        if (!over)
    ...
    ********************************************************************/
    ...
    setGeneralAppListener() {
        ...
        window.addEventListener('dragend', (e) => this.onDragEnd());
        ...
        document.addEventListener("dbltouchplayer", (e) => {
            var pos = this.scene.positionDblTapModule();
            this.createPlayerModule(pos.x, pos.y, (e.detail).player.ident);
        });
    }
    //used for Url pointing at a dsp file
    createModuleFromUrl(module, x, y, url) {
    ...
    uploadCodeFaust(module, x, y, dsp_code) {
        dsp_code = "process = vgroup(\"" + "TEXT" + "\",environment{" + dsp_code + "}.process);";
        if (!module) {
            this.compileFaust({ name: "TEXT",
                sourceCode: dsp_code,
                x: x,
                y: y,
                callback: (factory) => { this.createModule(factory); } });
    ...
    loadFileEvent(e) {
        Utilitary.showFullPageLoading();
        var file = e.detail;
        var position = this.scene.positionDblTapModule();
    ...
    onDragStart(evt) {
        ...
        var modules = this.scene.getModules();
        //for (var i = 0; i < modules.length; i++) {
    ...
    onDragEnd() {
        ...
        for (var module of this.scene.getModules())
            module.moduleView.fModuleContainer.classList.remove('dragover');
        this.menu.closeMenu();
    }
    onDrop(e, module) {
        e.preventDefault();
            ...
            switch (e.dataTransfer.getData('ddtype')) {
                ...
                case 'player':
                    ...
                    if (!module)
                        ...
                        break;
                default:
                    var dttext = e.dataTransfer.getData('text');
                    if (dttext)
    ...
    updateModuleFromFile(file, module) {
        this.loadFile(file, module, 0, 0);
    }
    createModulesFromFiles(files, x, y) {
        for (var i = 0; i < files.length; i++) {
            var f = files[i];
    ...
    createPlayerModule(x, y, url) {
        var player = this.players.getPlayer(url);
        var pm = new PlayerModule(Utilitary.idX++, x, y, 'player', document.getElementById('modules'), () => {
            pm.onremove(this);
            this.scene.removeModule(pm);
        }, this.compileFaust, this.audioContext, player);
    ...
    onMouseUp(e) {
        for (let node of [].map.call(document.querySelectorAll('.dragging'), (n) => n))
